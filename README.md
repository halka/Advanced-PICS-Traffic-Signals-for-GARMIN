...
- Shows receive status, last receive time, packet count, and current GPS position in a compact header
...
```
┌─────────────────────────────┐  ← Accent line
│                 ● 受信中     │  ← Header: receive status, right aligned
│ 05/17 21:20:42  8 pkt        │     Last receive time + packet count
│ 43.0668,141.3506             │     Current device position
├─────────────────────────────┤
│ 北５条通り札幌駅前交差点       │  ← Scrollable nearest-intersection cards
│ ID: SAPPORO_STN001           │     Type 1 BLE device/transmitter ID
│ 北海道札幌市中央区北五条西四丁目 │     Address, dynamically wrapped
│ Lat:43.0668 Lon:141.3506     │
│  ●     8           -46 dBm    │  ← Signal rows: circle + remaining value
│                  12m 45(NE)  │
└─────────────────────────────┘  ← No footer; cards use the remaining screen
```
...
┌─────────────────────────────┐  ← アクセントライン
│                 ● 受信中     │  ← ヘッダー: 受信状態を右寄せ
│ 05/17 21:20:42  8 pkt        │     最終受信日時 + 受信数
│ 43.0668,141.3506             │     自分の現在地
├─────────────────────────────┤
│ 北５条通り札幌駅前交差点       │  ← 近い交差点カードをスクロール表示
│ ID: SAPPORO_STN001           │     Type1由来のBLEデバイス/発信器ID
│ 北海道札幌市中央区北五条西四丁目 │     住所は長さから行数を決めて折り返し
│ Lat:43.0668 Lon:141.3506     │
│  ●     8           -46 dBm    │  ← 信号行: 丸 + 残り値
│                  12m 45(NE)  │
└─────────────────────────────┘  ← フッターなし。カードが残り画面を使用
